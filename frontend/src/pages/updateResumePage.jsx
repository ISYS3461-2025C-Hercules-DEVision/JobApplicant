...
// ✅ adjust path to your schema location
import { updateResumeSchema } from "../schemas/resume.schema";

function firstZodMessage(zodError) {
  return zodError?.issues?.[0]?.message || "Invalid form data.";
}
...
export default function UpdateResumePage({ onClose }) {
  ...
  const { resume: fetchedResume, loading, isSubmitting, updateResume } =
  ...
  // ✅ default form from fetched resume
  const defaultResume = useMemo(() => {
    return {
      ...
      certifications: Array.isArray(fetchedResume?.certifications)
        ? fetchedResume.certifications
        : [],

      education:
        fetchedResume?.education?.length > 0
      ...
      experience:
        fetchedResume?.experience?.length > 0
      ...
      mediaPortfolios: Array.isArray(fetchedResume?.mediaPortfolios)
        ? fetchedResume.mediaPortfolios
        : [],
  ...
  // ✅ IMPORTANT: sync when fetchedResume arrives later
  useEffect(() => {
  ...
  // ✅ Payload should be clean for backend
  const normalizePayload = () => {
    const minSalary =
      formData.minSalary === "" ? null : toNullableNumber(formData.minSalary);
    const maxSalary =
      formData.maxSalary === "" ? null : toNullableNumber(formData.maxSalary);

    const education = (formData.education || [])
      .map((e) => ({
        educationId: e.educationId || undefined,
        applicantId,
        institution: e.institution?.trim() || "",
        degree: e.degree?.trim() || "",
        fromYear: toNullableNumber(e.fromYear),
        toYear: toNullableNumber(e.toYear),
        gpa: toNullableNumber(e.gpa),
      }))
      .filter(
        (e) =>
          e.institution?.trim() ||
    ...
    const experience = (formData.experience || [])
      .map((x) => ({
        workExpId: x.workExpId || undefined,
        applicantId,
        jobTitle: x.jobTitle?.trim() || "",
        companyName: x.companyName?.trim() || "",
        fromYear: toNullableNumber(x.fromYear),
        toYear: toNullableNumber(x.toYear),
    ...
    return {
      ...
      education,
      experience,
      // ❌ don't send mediaPortfolios here unless your backend expects it
    };
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const parsed = updateResumeSchema?.safeParse
      ? updateResumeSchema.safeParse(payload)
    ...
    try {
      await updateResume(parsed.data);
      setSuccess(true);
      navigate("/profile");
    } catch (err) {
  ...
  // -----------------------------
  // Media (still stub unless you implement upload)
  // -----------------------------
  ...
  const handleUpload = async () => {
    ...
    // TODO: integrate your real upload logic (Cloudinary / backend)
    setUploading(true);
    try {
      // example: const newMedia = await uploadMedia(...)
      // setFormData(prev => ({...prev, mediaPortfolios: [...prev.mediaPortfolios, newMedia]}))
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-start justify-center p-4 z-50 overflow-y-auto scrollbar-hide">
      <div className="bg-white border-4 border-black p-10 w-full max-w-4xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] my-8">
        ...
        {success && (
          <div className="mb-6 p-4 border-4 border-green-600 bg-green-100 text-black font-bold">
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-3">
              {(!formData.mediaPortfolios ||
                formData.mediaPortfolios.length === 0) && (
                <p className="text-sm text-gray-500 font-semibold uppercase">
